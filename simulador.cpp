...
bool parseInput(std::string &input, std::string &address1,
                std::string &address2) {
  std::istringstream iss(input);
...
bool parseInput(std::string &input, std::string &address1,
                std::string &address2, std::string &address3) {
  std::istringstream iss(input);
...
void sumOperation(const std::string &address) {
  if (memory.find(address) != memory.end()) {
    ...
    std::cout << "La Direccion " << address << "No existe en la memoria."
              << std::endl;
  }
}
...
void sumOperation(std::string &address1, std::string &address2) {
  if (memory.find(address1) != memory.end() &&
      memory.find(address2) != memory.end()) {
    acc = memory[address1];
    acc += memory[address2];
  } else {
    std::cout << "La Direccion " << address1 << "No existe en la memoria."
              << std::endl;
  }
}
...
void sumOperation(std::string &address1, std::string &address2,
  ...
  if (memory.find(address1) != memory.end() &&
      memory.find(address2) != memory.end() &&
      memory.find(address3) != memory.end()) {
    acc = memory[address1];
    acc += memory[address2];
    memory[address3] = acc;
  } else {
...
void subOperation3(const std::string &address1, std::string &address2,
                   std::string &address3) {
  if (memory.find(address1) != memory.end() &&
      memory.find(address2) != memory.end() &&
      memory.find(address3) != memory.end()) {
...
void mulOperation(const std::string &address1, std::string &address2) {
  if (memory.find(address1) != memory.end() &&
    ...
    std::cout << "The addresses " << address1 << " " << address2
              << "are both not in memory" << std::endl;
  }
}
...
void store(const std::string &address) { memory[address] = acc; }
...
void beqOperation(const std::string &address) {
  if (memory.find(address) != memory.end()) {
    if (acc - memory[address] == 0) {
      memory[address] = acc;
    }
  } else {
    std::cout << "Memory Address not valid" << std::endl;
...
void beqOperation(const std::string &address1, const std::string &address2) {
  if (memory.find(address1) != memory.end() &&
      memory.find(address2) != memory.end()) {
    if (memory[address1] - memory[address2] == 0) {
      memory[address1] = memory[address2];
    }
  } else {
    std::cout << "Memory Address not valid" << std::endl;
  }
...
void beqOperation(const std::string &address1, const std::string &address2,
  ...
  if (memory.find(address1) != memory.end() &&
      ...
      memory.find(address3) != memory.end()) {
    if (memory[address1] - memory[address2] == 0) {
      memory[address3] = memory[address1];
    }
  } else {
    std::cout << "Memory Address not valid" << std::endl;
  }
...
void andOperation(const std::string &address1, const std::string &address2,
                  const std::string &address3) {
  if (memory.find(address1) != memory.end() &&
      memory.find(address2) != memory.end() &&
      memory.find(address3) != memory.end()) {
...
void orOperation(const std::string &address1, const std::string &address2,
                 const std::string &address3) {
  if (memory.find(address1) != memory.end() &&
      memory.find(address2) != memory.end() &&
      memory.find(address3) != memory.end()) {
      ...
      while (std::getline(file, input)) {
        ...
        if (command == "SET") {
          ...
          if (tokenCount == 2) {
            ...
            beqOperation(address);
          } else if (tokenCount == 3) {
            if (!parseInput(input, address1, address2)) {
              continue;
            }
            beqOperation(address1, address2);
          } else if (tokenCount == 4) {
            if (!parseInput(input, address1, address2, address3)) {
              continue;
            }
            beqOperation(address1, address2, address3);
          }
        } else {
